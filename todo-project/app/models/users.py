...
from app.models.mixin import UsersBaseModeMixin
from utils.messages import ERROR_MESSAGES

class User(UsersBaseModeMixin):
  def __init__(self):
  ...
  def select_user_by_email(self, email:str):    
    """ユーザー情報の取得"""
    return(
      self._base_query_by_email(self.session, email).first()
    )
  
  def select_user_by_id(self, user_id:int):    
    """ユーザー情報の取得"""
    return self._base_query_by_user_id(self.session, user_id).first()
    
  def update_user_by_id(self, user_id, name, email, password_hash):
    """ユーザー情報の更新"""
    user_to_update_by_id = self._base_query_by_user_id(self.session, user_id).first()
  ...
  def reset_password_hash_by_email(self, email, mew_password_hash):
    """ユーザー情報の更新"""
    user_to_update_by_email = self._base_query_by_email(self.session, email).first()
  ...
  def soft_delete_user_by_id(self, user_id: int):
    """ユーザー情報の論理削除"""
    user_to_soft_delete = self._base_query_by_user_id(self.session, user_id).first()
